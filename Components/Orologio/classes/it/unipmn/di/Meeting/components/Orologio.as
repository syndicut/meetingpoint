...
class it.unipmn.di.Meeting.components.Orologio extends UIObject implements IClientComponent, IUIWindowComponent {
	...
	function Orologio(clip:MovieClip) {
		super();
	...
	function setServerTime(yea) {
		...
		if(yea){
			...
			this.location.text = label:_root.stringsManager.get("CLOCK_SERVER_TIME");
			clearInterval(this.timerPing);
			...
			this.location.text = label:_root.stringsManager.get("CLOCK_LOCAL_TIME");
			clearInterval(this.timerPing);
	...
	function pingServer(from) {
		if(from.connector==undefined || from.connector==null){
			from._parent.newError(label:_root.stringsManager.get("CLOCK_ERROR_CONN"), true);
			return false;
		}
		else{
